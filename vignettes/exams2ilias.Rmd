...
targets the import structure validated against ILIAS 9.20.
...
Exercises bundled with `exams`, such as `"ttest.Rmd"` or `"boxplots.Rmd"`, can
be addressed directly by file name.

## Internal template

By default, the package uses the bundled ILIAS template alias `"ilias"`, which
currently resolves to the internal template for ILIAS 9.20. You only need to
set `template` explicitly if you want to use your own XML template file.

```{r}
exams2ilias(
  system.file("exercises", "stats_cloze.Rmd", package = "exams2ilias"),
  n = 1,
  dir = outdir,
  name = "stats_cloze",
  xmlcollapse = FALSE,
...
The package ships with self-contained statistics exercises for the main
question types:
...
These exercises are intended both as smoke-tested templates and as a starting
